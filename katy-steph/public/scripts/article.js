...
Article.prototype.insertRecord = function(callback) {
  $.post('/articles', {author: this.author, authorUrl: this.authorUrl, body: this.body, category: this.category, publishedOn: this.publishedOn, title: this.title})
    .then(data => {
      ...
      // The purpose is for once the insertRecord function is complete, it is looking for a callback. And if so, run the callback function. If we did have a callback function to do additional steps, then yes, it would be invoked. There is no callback function yet.
